...
export function Dropdown2D({ label = "Select", options, value, onSelect, className = "" }: Dropdown2DProps) {
    ...
    return (
        <div className={`relative inline-block text-left ${className}`.trim()}>
            <button
                type="button"
                onClick={() => setOpen((v) => !v)}
                className="inline-flex w-full items-center justify-between gap-3 rounded-xl border border-white/12 bg-slate-950 px-3 py-2 text-sm font-semibold text-slate-100 shadow-md backdrop-blur hover:border-white/30"
            >
                <span>{activeLabel}</span>
                <span className="text-xs uppercase tracking-[0.2em] text-slate-400">▼</span>
            </button>
            {open ? (
                <div className="absolute z-10 mt-2 w-full rounded-xl border border-white/12 bg-slate-950/95 p-2 shadow-2xl backdrop-blur">
