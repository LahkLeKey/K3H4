...
const vertexShader = /* glsl */ `
  ...
  void main() {
    vUv = uv;
    vec3 pos = position;
    float wobble = sin((pos.y + uTime * 2.0) * 6.0) * 0.02;
    pos.x += wobble;
    pos.z += wobble * 0.5;
    pos.y += sin(uTime * 3.0 + pos.x * 4.0) * 0.02;
...
const fragmentShader = /* glsl */ `
  ...
  void main() {
    vec2 uv = vUv;
    // stretch upward and add time-based distortion
    vec2 p = vec2(uv.x * 2.0 - 1.0, uv.y * 2.2);
    float t = uTime * 0.8;
    float n = fbm(vec2(p.x * 1.5, p.y * 3.0 - t * 2.0));
    float flame = smoothstep(0.2, 0.8, n + (1.0 - uv.y));

    vec3 base = mix(vec3(0.15, 0.08, 0.02), vec3(0.8, 0.24, 0.05), flame);
    vec3 hot = mix(base, vec3(1.0, 0.6, 0.2), smoothstep(0.6, 1.0, flame));
    float glow = smoothstep(0.4, 1.0, flame) * 0.5;
    vec3 color = hot + glow * vec3(1.0, 0.5, 0.1);
