...
export type PoiDetail = {
    ...
    route?: { distance: number; duration: number; mode: string; geometry?: any | null } | null;
    source?: { osmId: string; wikidataId?: string | null; lastUpdated?: string | null } | null;
} | null;
...
const DETAIL_BASE_INCLUDE = ["address", "contact", "openingHours", "fuel", "accessibility", "building", "photos", "description"];

const DETAIL_TTL_MS = 5 * 60_000;
const detailCache = new Map<string, { expiresAt: number; value?: PoiDetail; promise?: Promise<PoiDetail> }>();

const makeCacheKey = (id: string, include: string, origin: { lat: number; lng: number } | null) => {
    if (origin) return `${id}:${include}:${origin.lat.toFixed(4)},${origin.lng.toFixed(4)}`;
    return `${id}:${include}`;
};

export function usePoiDetailInteraction({ mapRef, apiBase, accessToken, signOut, poiPins, frame, origin }: Options) {
    ...
    useEffect(() => {
        const valid = origin && Number.isFinite(origin.lat) && Number.isFinite(origin.lng);
    ...
    const fetchPoiDetail = useCallback(
        async (poi: Poi) => {
            ...
            const includeList = [...DETAIL_BASE_INCLUDE, ...(includeRoute ? ["route"] : [])];
            const include = includeList.join(",");
            const cacheKey = makeCacheKey(id, include, includeRoute && originForRoute ? originForRoute : null);
            ...
            const request = (async () => {
                try {
                    ...
                    if (osmId) {
                        ...
                        const enrichUrl = `${apiBase}/api/poi/${osmId}/enrich?${qs.toString()}`;
                ...
                .then((value) => {
                    if (value) {
                        detailCache.set(cacheKey, { value, expiresAt: Date.now() + DETAIL_TTL_MS });
                    } else {
            ...
            detailCache.set(cacheKey, { promise: request, expiresAt: now + DETAIL_TTL_MS });
