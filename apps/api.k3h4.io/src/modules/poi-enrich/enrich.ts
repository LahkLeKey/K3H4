...
const NOMINATIM_URL = process.env.NOMINATIM_URL?.trim() || "https://nominatim.openstreetmap.org/reverse";
const NOMINATIM_USER_AGENT = process.env.NOMINATIM_USER_AGENT?.trim() || "k3h4-api/poi-enrich";
const WIKIMEDIA_LANG = process.env.WIKIMEDIA_LANG?.trim() || "en";
const BUILDING_SEARCH_RADIUS_M = 10;

const extractCenter = (el: any) => {
  if (el?.center?.lat && el?.center?.lon) return { lat: Number(el.center.lat), lon: Number(el.center.lon) };
  if (Number.isFinite(el?.lat) && Number.isFinite(el?.lon)) return { lat: Number(el.lat), lon: Number(el.lon) };
  return null;
};

const toFootprint = (el: any): Polygon | MultiPolygon | null => {
  const coords = Array.isArray(el?.geometry)
    ? el.geometry.map((p: any) => [Number(p.lon), Number(p.lat)] as [number, number])
    : null;
...
const pointInPolygon = (point: { lat: number; lon: number }, polygon: Polygon | null) => {
  ...
  for (let i = 0, j = ring.length - 1; i < ring.length; j = i++) {
    ...
    const intersect = yi > point.lat !== yj > point.lat && point.lon < ((xj - xi) * (point.lat - yi)) / denom + xi;
    if (intersect) inside = !inside;
  }
  return inside;
};

const extractAddressFromTags = (tags: Record<string, any> | undefined): Address | null => {
  if (!tags) return null;
  ...
  const neighborhood = tags["addr:neighbourhood"] ?? tags["addr:quarter"] ?? null;
...
const mergeAddress = (...addresses: Array<Address | null | undefined>): Address | null => {
...
const mapBuildingType = (tags: Record<string, any> | undefined): { type?: string | null; subtype?: string | null } => {
  if (!tags) return {};
  const raw = tags.building as string | undefined;
  if (!raw) return {};
  if (raw === "residential" || raw === "apartments" || raw === "house") return { type: "residential", subtype: raw };
  if (raw === "retail" || raw === "commercial" || raw === "office" || raw === "supermarket") return { type: "commercial", subtype: raw };
  return { type: raw, subtype: raw };
};

const extractFuelTypes = (tags: Record<string, any> | undefined): string[] | null => {
  if (!tags) return null;
  const fuels = Object.entries(tags)
    .filter(([key, value]) => key.startsWith("fuel:") && `${value}`.toLowerCase() === "yes")
    .map(([key]) => key.replace("fuel:", ""));
  return fuels.length > 0 ? fuels : null;
};

const extractContact = (tags: Record<string, any> | undefined): { phone?: string | null; website?: string | null } | null => {
  if (!tags) return null;
...
const buildOverpassElementQuery = (poi: PoiId) => {
  const query = `[out:json][timeout:25];(${poi.type}(${poi.id}););out tags center geom;`;
...
const buildOverpassBuildingQuery = (center: { lat: number; lon: number }) => {
  const query = `[out:json][timeout:25];way(around:${BUILDING_SEARCH_RADIUS_M},${center.lat},${center.lon})["building"];out tags geom;`;
  const body = new URLSearchParams({ data: query }).toString();
  const signature = `poi-enrich:building:${center.lat.toFixed(6)}:${center.lon.toFixed(6)}`;
  return { body, signature };
};
...
const reverseGeocode = async (center: { lat: number; lon: number }): Promise<Address | null> => {
  const url = `${NOMINATIM_URL}?${new URLSearchParams({ format: "jsonv2", lat: String(center.lat), lon: String(center.lon), addressdetails: "1" }).toString()}`;
...
const extractWikipediaFromTag = (tags: Record<string, any> | undefined): { lang: string; title: string } | null => {
...
const fetchWikipediaSummary = async (
  ...
  const url = `https://${wiki.lang}.wikipedia.org/w/api.php?${params.toString()}`;
  const res = await fetch(url, { headers: { "User-Agent": NOMINATIM_USER_AGENT } });
  ...
  const photo = img ? ({ url: img, caption: page?.title ?? null } as Photo) : null;
  return { description, photo };
};

const fetchCommonsImageUrl = async (fileName: string): Promise<string | null> => {
  const params = new URLSearchParams({ action: "query", prop: "imageinfo", iiprop: "url", format: "json", titles: `File:${fileName}` });
...
const fetchWikidataDescriptionAndPhoto = async (
  ...
  const { payload } = await fetchWikidataWithCache<any>(prisma, "/w/api.php", params, { resource: "poi-enrich", maxAgeMinutes: 60 * 24 * 7 });
...
export async function enrichPoi(
  ...
  const category = tags.amenity ?? tags.shop ?? tags.tourism ?? tags.leisure ?? tags.highway ?? null;
  ...
  if (flags.building) {
    ...
    const buildingSource = isBuildingElement ? base : await fetchContainingBuilding(center);
    if (buildingSource) {
      ...
      if (isBuildingElement || containsPoi) {
        const addrFromBuilding = extractAddressFromTags(buildingSource.tags ?? {});
  ...
  const accessibility = flags.accessibility ? { wheelchair: tags.wheelchair ?? null } : null;

  let route = null;
  if (flags.route && origin) {
    const coordinates = `${origin.lon},${origin.lat};${center.lon},${center.lat}`;
    const params: Record<string, string | number | boolean> = { overview: flags.routeGeometry ? "full" : "false", alternatives: 0 };
    if (flags.routeGeometry) params.geometries = "geojson";

    const { response } = await fetchOsrmWithCache(prisma, { service: "route", profile: mode, coordinates, params }, { maxAgeMinutes: 5 });
  ...
  if (flags.photos || flags.description) {
    ...
    if ((!description || photos.length === 0) && wikipediaTag) {
      const { description: wikiDesc, photo } = await fetchWikipediaSummary(wikipediaTag);
