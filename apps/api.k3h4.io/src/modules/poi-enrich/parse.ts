...
export function parseIncludeFlags(raw: unknown): IncludeFlags {
  ...
  if (list.length === 0 || list.includes("all")) return { ...DEFAULT_INCLUDE_FLAGS };
  ...
  const flags: IncludeFlags = {
    ...
    routeGeometry: normalized.has("routegeometry") || normalized.has("geometry"),
    photos: normalized.has("photos"),
    description: normalized.has("description") || normalized.has("photos"),
  };

  // Always include name/category base info implicitly; ensure at least photos implies description
  return flags;
}
...
export function parseOrigin(query: Record<string, unknown>): OriginPoint | null {
  ...
  const lon = Number(query.originLon ?? query.lon ?? query.longitude ?? query.lng);
  if (!Number.isFinite(lat) || !Number.isFinite(lon)) return null;
  return { lat, lon };
}

export function computeIncludeHash(flags: IncludeFlags, origin: OriginPoint | null, mode: string): string {
  const payload = {
    include: INCLUDE_ALL.reduce((acc, key) => {
      acc[key] = flags[key];
      return acc;
