...
type DeleteJobStatus = {
  ...
  progress: number;
  message?: string;
  counts?: Record<string, unknown>; updatedAt: number;
};
...
const fetchGithubUserProfileWithRetry = async(
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    const ghUser =
        (await userRes.json().catch(() => ({}))) as GitHubUserProfile;
    lastResponse = userRes;
    lastUser = ghUser;

    const badCredentials =
        userRes.status === 401 && ghUser.message === 'Bad credentials';
    if (userRes.ok && ghUser.id) {
      if (attempt > 0) {
        logger.debug(
            {attempt: attempt + 1}, 'github user fetch succeeded after retry');
      }
    ...
    logger.debug(
        {attempt: attempt + 1}, 'github user fetch bad credentials, retrying');
    await wait(baseDelayMs * (attempt + 1));
  }

  return {
    userRes: lastResponse!,
    ghUser: lastUser ?? ({} as GitHubUserProfile)
  };
};
...
const createSessionTokens = async (
    server: FastifyInstance, prisma: PrismaClient, userId: string,
    email?: string) => {
...
const runDeleteJob = async (
    prisma: PrismaClient, jobId: string, userId: string,
    recordTelemetry: RecordTelemetryFn, request: any) => {
  const updateStatus = (partial: Partial<DeleteJobStatus>) => {
    const current = deletionJobs.get(jobId);
    if (!current) return;
    const next = {...current, ...partial, updatedAt: Date.now()} as
        DeleteJobStatus;
  ...
  try {
    updateStatus(
        {status: 'running', progress: 10, message: 'Starting deletion'});
    ...
    updateStatus(
        {status: 'done', progress: 100, message: 'Deletion complete', counts});
...
export function registerAuthRoutes(
  ...
  server.post('/auth/github/callback', async (request, reply) => {
    ...
    try {
      ...
      const {userRes, ghUser} =
          await fetchGithubUserProfileWithRetry(accessToken, request.log);
      if (!userRes.ok || !ghUser.id) {
        await rt({
          eventType: 'auth.github.callback.failed',
          source: 'api',
      ...
      if (!primaryEmail) {
        try {
          ...
          if (emailRes.ok) {
            ...
            const errBody =
                (await emailRes.json().catch(() => ({}))) as {message?: string};
            return reply.status(401).send({
              error: 'Unable to fetch GitHub email',
              detail: errBody.message ?? null,
      ...
      if (!primaryEmail) {
        return reply.status(401).send({
          error:
              'GitHub email not available; ensure user:email scope and a verified email'
        });
      }
      ...
      return createSessionTokens(
          server, prisma, user.id, user.email ?? undefined);
    } catch (err) {
  ...
  server.post('/auth/github/url', async (request, reply) => {
    ...
    const authorizeUrl = `https://github.com/login/oauth/authorize?client_id=${
        ...
        encodeURIComponent(
            redirectUri)}&scope=read:user%20user:email&prompt=login`;
    return {authorizeUrl};
  });
  ...
  server.post('/auth/linkedin/url', async (request, reply) => {
    ...
    const authorizeUrl =
        `https://www.linkedin.com/oauth/v2/authorization?response_type=code&client_id=${
            encodeURIComponent(clientId)}&redirect_uri=${
            encodeURIComponent(
                redirectUri)}&scope=openid%20profile%20email&prompt=login&state=${
            state}`;
  ...
  server.post('/auth/linkedin/callback', async (request, reply) => {
    ...
    if (!tokenRes.ok || !tokenData.access_token) {
      return reply.status(401).send(
          {error: tokenData.error || 'LinkedIn auth failed'});
    }
      ...
      try {
        ...
        if (emailRes.ok) {
          ...
          const errBody =
              (await emailRes.json().catch(() => ({}))) as {message?: string};
          request.log.warn(
              {status: emailRes.status, body: errBody},
    ...
    if (!primaryEmail) {
      return reply.status(401).send({
        error:
            'LinkedIn email not available; ensure r_emailaddress scope and a verified email'
      });
    }
  ...
  server.post(
      ...
      async (request, reply) => {
        ...
        try {
          ...
          if (!body?.confirmText ||
              body.confirmText !== ACCOUNT_DELETE_PHRASE) {
            return reply.status(400).send({
              error: `To delete your data, type '${
                  ACCOUNT_DELETE_PHRASE}' exactly.`
            });
          }
          ...
          await rt({
            ...
            source: 'api',
            payload: {
              confirmationMatched: body.confirmText === ACCOUNT_DELETE_PHRASE
            }
          });
    ...
    try {
      ...
      try {
        const auth = (request.headers.authorization as string) ||
            (request.headers.Authorization as unknown as string) || '';
            ...
            if (clientId && clientSecret && g.accessToken) {
              ...
              await fetch(
                  `https://api.github.com/applications/${
                      encodeURIComponent(clientId)}/token`,
                  {
                    method: 'DELETE',
                    headers: {
