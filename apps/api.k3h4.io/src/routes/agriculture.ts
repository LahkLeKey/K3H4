...
const serializePlan = (plan: any) => ({
  ...
  targetHarvestDate:
      plan.targetHarvestDate ? plan.targetHarvestDate.toISOString() : null,
...
const serializePlot = (plot: any) => {
  ...
  return {
    ...
    notes: plot.notes,
    lastConditionAt: plot.lastConditionAt ? plot.lastConditionAt.toISOString() :
                                            null,
    latestCondition: latestCondition ? {
      ...
      ph: latestCondition.ph instanceof Prisma.Decimal ?
          latestCondition.ph.toFixed(2) :
          null,
    ...
    plans: plot.cropPlans ?
        plot.cropPlans.map(
            (plan: any) => ({
              ...
              status: plan.status,
              startDate: plan.startDate.toISOString(),
              targetHarvestDate: plan.targetHarvestDate ?
                  plan.targetHarvestDate.toISOString() :
                  null,
              endDate: plan.endDate ? plan.endDate.toISOString() : null,
            })) :
        [],
...
const serializeInventory = (inventory: any) => ({
  ...
  totalQuantity: inventory.totalQuantity instanceof Prisma.Decimal ?
      inventory.totalQuantity.toFixed(2) :
      '0.00',
  unit: inventory.unit,
  location: inventory.location,
  status: inventory.status,
  movements: inventory.movements ?
      inventory.movements.map(
          (movement: any) => ({
            id: movement.id,
            type: movement.type,
            quantity: movement.quantity instanceof Prisma.Decimal ?
...
const serializeSlot = (slot: any) => ({
  ...
  costPaid: slot.costPaid instanceof Prisma.Decimal ? slot.costPaid.toFixed(2) :
                                                      '0.00',
...
export function registerAgricultureRoutes(
  ...
  server.get(
      ...
      async (request) => {
        ...
        const [plots, tasks, shipments, slots, inventorySummary] =
            await Promise.all([
        ...
        const inventory = inventorySummary.reduce((acc, row) => {
          acc[row.sku.toLowerCase()] =
              row._sum.totalQuantity instanceof Prisma.Decimal ?
              row._sum.totalQuantity.toFixed(2) :
              '0.00';
          return acc;
  ...
  server.get(
      ...
      async (request) => {
        ...
        const slots = await prisma.agricultureSlot.findMany({
          ...
          include:
              {plot: {select: {id: true, name: true, crop: true, stage: true}}},
        });
        await rt({
  ...
  server.post(
      ...
      async (request, reply) => {
        ...
        const unlockedCount =
            await prisma.agricultureSlot.count({where: {userId}});
        ...
        await rt({
          ...
          payload: {
            slotIndex,
            costPaid: costPaid instanceof Prisma.Decimal ? costPaid.toString() :
                                                           `${costPaid}`
          },
        });
  ...
  server.patch(
      ...
      async (request, reply) => {
        ...
        const nextPlotId =
            body?.plotId === undefined ? slot.plotId : body.plotId ?? null;
        if (nextPlotId) {
          const plot = await prisma.agriculturePlot.findFirst(
              {where: {id: nextPlotId, userId}});
          if (!plot)
            return reply.status(404).send({error: 'Plot not found for slot'});
        }
  ...
  server.post(
      ...
      async (request, reply) => {
        ...
        const plot = await prisma.$transaction(async (tx) => {
          ...
          const openSlot = await tx.agricultureSlot.findFirst(
              {where: {userId, plotId: null}, orderBy: {slotIndex: 'asc'}});
          if (openSlot) {
            await tx.agricultureSlot.update(
                {where: {id: openSlot.id}, data: {plotId: createdPlot.id}});
          }
          ...
          const condition = await tx.agriculturePlotCondition.create({
            data: {
              ...
              temperature: typeof body?.temperature === 'number' ?
                  body.temperature :
                  null,
              moisture: typeof body?.moisture === 'number' ? body.moisture :
                                                             null,
              ph: toDecimal(body?.ph),
              notes: body?.notes ?? null,
            },
          });
          await tx.agriculturePlot.update(
              {where: {id: plotId}, data: {lastConditionAt: recordedAt}});
          return condition;
        });
        ...
        const plan = await prisma.agricultureCropPlan.create({
          data: {
            ...
            targetHarvestDate: body.targetHarvestDate ?
                new Date(body.targetHarvestDate) :
                null,
  ...
  server.get(
      ...
      async (request, reply) => {
        ...
        if (query.status) {
          const statusFilter = parseLifecycleStatus(query.status);
          if (!statusFilter) {
            return reply.status(400).send({error: 'Invalid status filter'});
          }
          where.status = statusFilter;
        }
        const tasks = await prisma.agricultureTask.findMany(
  ...
  server.post(
      ...
      async (request, reply) => {
        ...
        if (body?.cropPlanId &&
            !(await prisma.agricultureCropPlan.findFirst(
                {where: {id: body.cropPlanId, userId}}))) {
          return reply.status(404).send({error: 'Crop plan not found'});
        }

        const priorityValue =
            body?.priority !== undefined ? Number(body.priority) : NaN;
        const normalizedPriority = Number.isFinite(priorityValue) ?
  ...
  server.patch(
      ...
      async (request, reply) => {
        ...
        const task = await prisma.agricultureTask.findFirst(
        ...
        if (Number.isFinite(requestedPriority)) {
          updates.priority =
              Math.max(1, Math.min(5, Math.floor(requestedPriority)));
        }
        if (body?.dueDate) {
        ...
        const updated = await prisma.agricultureTask.update(
            {where: {id: taskId}, data: updates});
        await rt({
  ...
  server.post(
      ...
      async (request, reply) => {
        ...
        const inventory = await prisma.agricultureInventory.findFirst(
            {where: {id: body.inventoryId, userId}});
        if (!inventory)
        ...
        const movement = await prisma.$transaction(async (tx) => {
          ...
          await tx.agricultureInventory.update(
              {where: {id: inventory.id}, data: {totalQuantity: nextTotal}});
          return created;
        });
        ...
        return serializeInventory(
            {...inventory, totalQuantity: nextTotal, movements: [movement]});
      },
  );
  ...
  server.get(
      ...
      async (request) => {
        const rt = withTelemetryBase(recordTelemetry, request);
        const userId = (request.user as {sub: string}).sub;
        const [totalPlots, planPhases, taskStatuses, inventoryItems, shipmentCount] =
            await Promise.all([
              ...
              prisma.agricultureInventory.findMany(
                  {where: {userId}, orderBy: {totalQuantity: 'desc'}, take: 3}),
              prisma.agricultureShipment.count(
                  {where: {userId, eta: {not: null}}}),
            ]);
        ...
        return {
          ...
          inventoryHighlights: inventoryItems.map(
              (item) => ({
                sku: item.sku,
