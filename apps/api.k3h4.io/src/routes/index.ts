...
type Registrar = (server: FastifyInstance, prisma: PrismaClient, recordTelemetry: RecordTelemetryFn) => void | Promise<void>;
...
const isRouteFile = (name: string) => {
    return name.endsWith(".ts") || name.endsWith(".js") || name.endsWith(".mjs") || name.endsWith(".mts");
};

export async function registerAllRoutes(server: FastifyInstance, prisma: PrismaClient, recordTelemetry: RecordTelemetryFn) {
    ...
    for (const entry of entries) {
        ...
        const candidates = Object.entries(mod).filter(([key, val]) =>
            typeof val === "function" && key.startsWith("register") && key.endsWith("Routes")
        );

        for (const [key, fn] of candidates) {
            const base = key.replace(/^register/i, "").replace(/Routes$/i, "").toLowerCase();
            const weight = PRIORITY[base] ?? 1000;
            registrars.push({ name: base || entry.name, fn: fn as Registrar, weight });
        }
    }

    registrars.sort((a, b) => a.weight - b.weight || a.name.localeCompare(b.name));
