...
const serializeShift = (shift: any) => ({
  ...
  assignedCandidate: shift.assignedCandidate
    ? { id: shift.assignedCandidate.id, fullName: shift.assignedCandidate.fullName, stage: shift.assignedCandidate.stage }
    : null,
});
...
export function registerStaffingRoutes(server: FastifyInstance, prisma: PrismaClient, recordTelemetry: RecordTelemetryFn) {
  server.get(
    "/staffing/dashboard",
    ...
    async (request) => {
      ...
      const [engagements, roles, candidates, shifts, placements] = await Promise.all([
        prisma.staffingEngagement.findMany({
          where: { userId },
          orderBy: { createdAt: "desc" },
          include: {
            roles: { orderBy: { createdAt: "desc" }, include: { placements: true, shifts: true } },
            candidates: { orderBy: { updatedAt: "desc" }, include: { persona: { select: { id: true, alias: true, account: true, handle: true } } } },
            placements: true,
          },
        }),
        ...
        prisma.staffingCandidate.findMany({
          ...
          include: { persona: { select: { id: true, alias: true, account: true, handle: true } }, role: true },
        }),
        prisma.staffingShift.findMany({
          ...
          include: {
            role: true,
            assignedPersona: { select: { id: true, alias: true, account: true, handle: true } },
            assignedCandidate: { select: { id: true, fullName: true, stage: true } },
          },
        }),
      ...
      const totalOpenings = roles.reduce((sum, role) => sum + (role.openings || 0), 0);
      const totalFilled = roles.reduce((sum, role) => sum + (role.filled || 0), 0);
      const fillRate = totalOpenings > 0 ? Number(((totalFilled / totalOpenings) * 100).toFixed(1)) : 0;

      const metrics = {
        openRoles: roles.filter((r) => r.status !== "closed").length,
        activeCandidates: candidates.filter((c) => c.stage !== "archived").length,
  ...
  server.post(
    ...
    async (request, reply) => {
      const userId = (request.user as { sub: string }).sub;
      const body = request.body as { name?: string; client?: string; priority?: string; status?: string; startDate?: string; endDate?: string; budget?: number | string; forecast?: number | string; notes?: string } | undefined;
  ...
  server.post(
    ...
    async (request, reply) => {
      const userId = (request.user as { sub: string }).sub;
      const body = request.body as { engagementId?: string; title?: string; location?: string; modality?: string; openings?: number | string; rateMin?: number | string; rateMax?: number | string; billRate?: number | string; payRate?: number | string; priority?: string; status?: string; tags?: string; skills?: string[] | undefined } | undefined;

      const title = body?.title?.trim();
      if (!title) return reply.status(400).send({ error: "title is required" });

      if (body?.engagementId) {
        const engagement = await prisma.staffingEngagement.findFirst({ where: { id: body.engagementId, userId } });
        if (!engagement) return reply.status(404).send({ error: "Engagement not found" });
      }

      const openings = body?.openings !== undefined ? Number(body.openings) : 1;
      if (!Number.isFinite(openings) || openings <= 0) return reply.status(400).send({ error: "openings must be positive" });
  ...
  server.post(
    ...
    async (request, reply) => {
      const userId = (request.user as { sub: string }).sub;
      const body = request.body as { fullName?: string; email?: string; phone?: string; source?: string; stage?: string; score?: number | string; desiredRate?: number | string; availability?: string; location?: string; note?: string; tags?: string[]; engagementId?: string; roleId?: string; personaId?: string } | undefined;

      const fullName = body?.fullName?.trim();
      if (!fullName) return reply.status(400).send({ error: "fullName is required" });
      ...
      if (body?.roleId) {
        const role = await prisma.staffingRole.findFirst({ where: { id: body.roleId, userId } });
        if (!role) return reply.status(404).send({ error: "Role not found" });
      }
      if (body?.personaId) {
        const persona = await prisma.persona.findFirst({ where: { id: body.personaId, userId } });
      ...
      const candidate = await prisma.staffingCandidate.create({
        data: {
          ...
          score: body?.score !== undefined && body?.score !== null && body.score !== "" ? new Prisma.Decimal(Number(body.score).toFixed(2)) : null,
        ...
        include: { persona: { select: { id: true, alias: true, account: true, handle: true } } },
      });
  ...
  server.post(
    ...
    async (request, reply) => {
      ...
      const candidate = await prisma.staffingCandidate.findFirst({ where: { id: candidateId, userId }, include: { persona: { select: { id: true, alias: true, account: true, handle: true } } } });
      const owned = await assertUserOwns(candidate, reply, "Candidate not found");
      if (!owned) return;
  ...
  server.post(
    ...
    async (request, reply) => {
      const userId = (request.user as { sub: string }).sub;
      const body = request.body as { roleId?: string; title?: string; location?: string; startsAt?: string; endsAt?: string; status?: string; coverageStatus?: string; assignedPersonaId?: string; assignedCandidateId?: string; notes?: string } | undefined;
      ...
      if (!startsAt || !endsAt || startsAt >= endsAt) return reply.status(400).send({ error: "startsAt and endsAt must be valid and ordered" });
      ...
      if (body?.assignedPersonaId) {
        const persona = await prisma.persona.findFirst({ where: { id: body.assignedPersonaId, userId } });
        if (!persona) return reply.status(404).send({ error: "Persona not found" });
      }
      if (body?.assignedCandidateId) {
        const candidate = await prisma.staffingCandidate.findFirst({ where: { id: body.assignedCandidateId, userId } });
      ...
      const shift = await prisma.staffingShift.create({
        data: {
          ...
          coverageStatus: body?.coverageStatus?.trim() || "unfilled",
  ...
  server.post(
    ...
    async (request, reply) => {
      const userId = (request.user as { sub: string }).sub;
      const body = request.body as { engagementId?: string; roleId?: string; candidateId?: string; personaId?: string; startDate?: string; endDate?: string; status?: string; billRate?: number | string; payRate?: number | string; note?: string } | undefined;

      const startDate = parseDate(body?.startDate);
      if (!startDate) return reply.status(400).send({ error: "startDate is required" });
      ...
      if (body?.candidateId) {
        const candidate = await prisma.staffingCandidate.findFirst({ where: { id: body.candidateId, userId } });
        if (!candidate) return reply.status(404).send({ error: "Candidate not found" });
      }
      if (body?.personaId) {
        const persona = await prisma.persona.findFirst({ where: { id: body.personaId, userId } });
