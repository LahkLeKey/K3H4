...
await server.register(fastifyRateLimit, {
  // Higher overall limits for internal DB-backed routes; per-route limits will
...
if (existsSync(docsStaticDir)) {
  ...
  server.log.warn(
      {docsStaticDir}, 'swagger ui static dir missing; using built-in assets');
}

await server.register(fastifySwaggerUi, swaggerUiOpts as any);
const corsOrigins = process.env.CORS_ORIGIN?.split(',') ?? [
  'http://localhost:5173', 'http://localhost:4173', 'https://www.k3h4.dev',
  'https://api.k3h4.dev'
];
